...
public class AuthAPI(IHttpClientFactory factory) : AuthenticationStateProvider
{
    ...
    public async Task<AuthResponse> CreateAsync(string email, string password)
    {
        var response = await _httpClient.PostAsJsonAsync("auth/create?useCookies=true", new 
        { 
            email,
            password           
        });
        ...
        return new AuthResponse { Sucesso = false, Erros = [$"Erro ao executar o registro do usuário"] };
    }
